...
public class WorkflowApp {
    ...
    public static void main(String[] args) throws GraphStateException {
        ...
        WorkflowContent initialContext = WorkflowContent.builder()
                .originalPrompt("创建一个人博客网站,主题为旅游")
                .currentStep("初始化")
                .build();
        ...
        // 执行工作流
        int stepCounter = 1;
        for (NodeOutput<MessagesState<String>> step : workflow.stream(Map.of(WorkflowContent.WORKFLOW_CONTEXT_KEY, initialContext))) {
            log.info("--- 第 {} 步完成 ---", stepCounter);
            // 显示当前状态
            WorkflowContent currentContext = WorkflowContent.getContext(step.state());
